...
class ExampleBloc extends Bloc<ExampleEvent, ExampleState> {
  ...
  FutureOr<void> _insertName(ExampleAddNameEvent event, Emitter<ExampleState> emit) {
    final stateExample = state;
    if (stateExample is ExampleStateData) {
      final names = stateExample.names;
      final update = List<String>.from(names)..add(event.name);

      emit(ExampleStateData(names: update));
    }
  }
